...
class ModelSpec:
    ...
    min_points: int
    fn: Callable[[np.ndarray, int], np.ndarray | None]
    intermittent_only: bool = False
    heavy: bool = False          # MLE-fitted: competed only on long lookbacks

    def fit_predict(self, train: np.ndarray, horizon: int) -> np.ndarray | None:
        if len(train) < self.min_points:
            return None
        try:
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                out = self.fn(np.asarray(train, dtype=float), horizon)
        except Exception:
            return None
        if out is None:
            return None
...
def _weighted_moving_average(train, h, k=4):
